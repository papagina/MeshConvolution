...
class Parameters():
    ...
    def read_config(self, fn):
        ...
        self.start_epoch=int(config.get("Params","start_iter"))
        ...
        self.use_vanilla_pool =0 #0 for using mcPool, 1 for avg pool, 2 for max pool
        try:
            self.use_vanilla_pool = json.loads(config.get("Params", "use_vanilla_pool"))
        except:
            1+1
